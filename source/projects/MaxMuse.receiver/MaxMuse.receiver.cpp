...
class receiver : public object<receiver> {
  private:
    ...
    // the number of outlers is defined at runtime depending on the number of
    // channels of the LSL inlet
  ...
  public:
    ...
    receiver(const atoms &args = {}) : lsl_inlet(nullptr) {
        ...
        if (results.empty())
            ...
            // Define the number of outlets depending on the number of channels
            // of the LSL inlet
            ...
            for (auto i = 0; i < n_channels; ++i) {
                auto an_outlet = std::make_unique<outlet<>>(
                    this,
                    "(float) " + info.type() + std::to_string(i + 1) + "");
    ...
    void stopThreads() { stopThreadFlag = true; }
    ...
    timer<> deliverer{this, MIN_FUNCTION{std::vector<float> sample;
